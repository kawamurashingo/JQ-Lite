...
on the changed edge cases until they can require 3.0. In particular:
...
| `path |= filter` expecting only the updated leaf | append the path projection after the update |
