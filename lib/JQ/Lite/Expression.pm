    ...
    my $value = eval {
        _eval_node(
            $ast,
            {
                ...
                resolve_variable => $opts{resolve_variable},
...
sub _nud {
    ...
    return { type => 'VARIABLE', name => $token->{name}, suffix => $token->{suffix} } if $token->{type} eq 'VARIABLE';
...
sub _eval_node {
    ...
    if ($node->{type} eq 'VARIABLE') {
        my $resolver = $opts->{resolve_variable} || sub { return undef };
        return $resolver->($node->{name}, $node->{suffix});
    }
...
1;
            ...
            if (substr($expr, $i) =~ /^\$([A-Za-z_][A-Za-z0-9_]*)([^\s+\-*\/%(),]*)/) {
                my ($name, $suffix) = ($1, $2 // '');
                push @tokens, { type => 'VARIABLE', name => $name, suffix => $suffix };
                $i += 1 + length($name) + length($suffix);
                next;
            }
            return;
        }
